...
class Window2(QDialog):
        ...
        while self.FTS_THREAD:
            try:
                ...
                if status == "ERROR" and result == "NA" and macaddress == "0":
                    ...
                    print("current_id and get ftsid:", self.current_id, ftsid)
    ...
    def update_ui_and_info(self, data: dict):
        ...
        try:
            ...
            if msg == "FTS is testing":
                ...
                self.info_show.setText("请等待单板FTS测试完成")
                ...
                if ftsresult == "SUCCESS":
                    ...
                    self.info_show.setText("当前单板测试通过，请开始下一台测试")
                    self.info_show.append("(详情请参考SOP)")
    ...
    def upload_fts_data_to_tn4cio(self, macaddress, ftsresult):
        ...
        ret = self.net.upload_mac_and_fts(fts_mac, fts_result)
        ...
        if ret != None:
            ...
            if msg_type == "ok":
                ...
                if msg == "db is full!":
                    self.info_show.setText("上传MAC: "+fts_mac+" 失败,数据库已经无可用空间!")
                elif msg == "upload mac and pcba fts fail!":
                    self.info_show.setText("上传MAC: "+fts_mac+" 失败!")
                elif msg == "only update pcba fts result fail!":
                    self.info_show.setText("更新FTS测试数据: "+fts_result+" 失败!")
    ...
    def show_pcbafts_tested_info(self):
        try:
            ret = self.net.get_pcbafts_tested_info()
            total = ret['total']
            produced = ret['produced']
            pass_num = ret['pcbafts_pass_count']
            fail_num = ret['pcbafts_fail_count']
    ...
    def handle_cmd(self):
        ...
        if cmd == "CMD_FTS_FAIL_CONFIRM":
            ...
            self.info_show.append("1.命令:"+cmd+" 不支持!")
            self.info_show.append("2.即将在2秒后回到初始状态")
