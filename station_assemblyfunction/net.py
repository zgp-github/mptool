...
class network():
    ...
    def get_tested_sensors_info(self):
        ...
        body = {"pokey": self.pokey, "countrycode": self.countrycode, "hwversion": self.hwversion}
        tmp = str(random.randint(1, 1000))
        url = "http://" + self.tn4cioip + "/tn4cio/srv/copies_NGxx/app.php/get_function_test_sensors_info/" + tmp
        try:
            response = requests.post(url, data=json.dumps(body), headers=headers, timeout=5)
            ret = response.text
            logging.debug(ret)
    ...
    def upload_pairTest_result(self, mac, val):
        headers = {'content-type': "application/json"}
        body = {"pokey": self.pokey, "countrycode": self.countrycode, "hwversion": self.hwversion,
                "macaddress": mac, "pairresult": val}
        tmp = str(random.randint(1, 1000))
        url = "http://" + self.tn4cioip + "/tn4cio/srv/copies_NGxx/app.php/upload_pairTest_result/" + tmp
        try:
            response = requests.post(url, data=json.dumps(body), headers=headers, timeout=5)
            ret = response.text
            logging.debug(ret)
    ...
    def upload_functionTest_result(self, mac, val):
        headers = {'content-type': "application/json"}
        body = {"pokey": self.pokey, "countrycode": self.countrycode, "hwversion": self.hwversion,
                "macaddress": mac, "functionresult": val}
        tmp = str(random.randint(1, 1000))
        url = "http://" + self.tn4cioip + "/tn4cio/srv/copies_NGxx/app.php/upload_functionTest_result/" + tmp
        try:
            response = requests.post(url, data=json.dumps(body), headers=headers, timeout=5)
            ret = response.text
            logging.debug(ret)
    ...
    def upload_temperature_to_corelight(self, macaddress, temperature, reftemperature):
        headers = {'content-type': "application/json"}
        body = {"pokey": self.pokey, "countrycode": self.countrycode, "hwversion": self.hwversion,
                "macaddress": macaddress, "temperature": temperature, "reftemperature": reftemperature}
        tmp = str(random.randint(1, 1000))
        url = "http://" + self.tn4cioip + "/tn4cio/srv/copies_NGxx/app.php/upload_temperature/" + tmp
        try:
            response = requests.post(url, data=json.dumps(body), headers=headers, timeout=5)
            ret = response.text
            logging.debug(ret)
    ...
    def check_previous_station_already_done(self, macaddress: str) -> bool:
        ...
        try:
            ...
            result = text['result']
            if result == "ok":
                pcbafts = text['messages'][0]['pcbafts']
                if pcbafts == "none":
                    return False
                else:
                    return True
