...
class network():
    ...
    def get_tested_sensors_info(self):
        ...
        body = {"pokey": self.pokey, "countrycode": self.countrycode, "hwversion": self.hwversion}
        tmp = str(random.randint(1, 1000))
        url = "http://"+self.tn4cioip+"/tn4cio/srv/copies_NGxx/app.php/get_ngstb_iqa_tested_info/"+tmp
        try:
            response = requests.post(url, data=json.dumps(body), headers=headers, timeout=5)
            ret = response.text
            logging.debug(ret)
    ...
    def upload_iqa_to_tn4cio(self, mac, val):
        headers = {'content-type': "application/json"}
        body = {"pokey": self.pokey, "countrycode": self.countrycode, "hwversion": self.hwversion,
                "macaddress": mac, "ngstbiqa": val}
        tmp = str(random.randint(1, 1000))
        url = "http://"+self.tn4cioip+"/tn4cio/srv/copies_NGxx/app.php/update_ngstb_iqa_data/"+tmp
        try:
            response = requests.post(url, data=json.dumps(body), headers=headers, timeout=5)
            ret = response.text
            logging.debug(ret)
